...
if (version_compare(phpversion(), '5.3.1', '<')) {
    if (substr(phpversion(), 0, 5) != '5.3.1') {
        ...
        echo "PEAR2_Pyrus_WebFrontend requires PHP 5.3.1 or newer.
";
        exit -1;
    }
}
...
try {
    ...
    echo "Cannot process PEAR2_Pyrus_WebFrontend phar:
";
    echo $e->getMessage(), "
";
    exit -1;
...
function PEAR2_Pyrus_WebFrontend_autoload($class)
{
    ...
    if (file_exists('phar://' . __FILE__ . '/PEAR2_Pyrus_WebFrontend-@PACKAGE_VERSION@/php/' . implode('/', explode('\\', $class)) . '.php')) {
        include 'phar://' . __FILE__ . '/PEAR2_Pyrus_WebFrontend-@PACKAGE_VERSION@/php/' . implode('/', explode('\\', $class)) . '.php';
    }
}
spl_autoload_register("PEAR2_Pyrus_WebFrontend_autoload");
$phar = new Phar(__FILE__);
$sig = $phar->getSignature();
define('PEAR2_Pyrus_WebFrontend_SIG', $sig['hash']);
define('PEAR2_Pyrus_WebFrontend_SIGTYPE', $sig['hash_type']);
